...
- 시각자료가 관찰하려는 행동을 보여 주지 못하는 문항 3건
...
발행은 문항 ID뿐 아니라 **선택지 ID까지** 영구히 고정한다. 학생 응답이
선택지 ID 를 참조하기 때문에, `protect_published_diagnosis` 는 새
발행본을 같은 set_key 의 *모든* 기존 행과 대조해 하나라도 사라지면
거부한다. 발행된 뒤에는 오답의 값을 바꿔도 그 ID 는 옛 값을 이름으로
유지할 수밖에 없다.

`2.2.0` 에는 그런 선택지가 셋 있다.

| 문항 | 선택지 ID | 실제 라벨 |
| --- | --- | --- |
| `g3s2-circle-08` | `25cm` | 그릴 때마다 달라져요 |
| `g3s2-div-10` | `about-80-each` | 200개쯤 |
| `g3s2-div-08` | `7x5-plus6` | 6+7+5=18 |

**응답 원본을 눈으로 볼 때는 반드시 해당 버전의 콘텐츠와 join 해야
한다.** ID 만 읽으면 학생이 고르지 않은 답을 골랐다고 오해한다.

이 제약은 문항을 고치는 방법도 바꾼다. `g3s2-graph-06` 은 표를 원자료
그림으로 바꾸면서도 마릿수를 `2.1.0` 과 같게 두어 세 선택지 ID 를 모두
살렸다. 시각 자료와 지문은 자유롭게 바꿀 수 있으니, 값을 유지할 수
있으면 그렇게 하는 편이 낫다.
