...
세 검수가 모두 통과시킨 문항에서도 결함이 나왔다. `g3s2-div-01`의 오답
`2개`는 도출이 자릿수 누락인데 오개념 ID는 `partial-value-used-as-result`라
서로 맞지 않는다. 이 문항은 학생에게 배정된 운영본 `1.0.0`에 들어 있어
버전 간 문항이 갈리지 않도록 손대지 않았다. 검수 에이전트의 통과 판정은
근거를 재계산해 확인하기 전까지 증거가 아니다.
