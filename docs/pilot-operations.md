...
`grade3-semester2@2.1.0`과 그 내용 검수 수정본 `2.2.0`은 `review`
상태이며 교사의 내용 검토와 별도 발행 결정을 마치기 전에는 배정할 수
없다. 발행된 `2.1.0`은 고칠 수 없으므로 검수 수정은 항상 새 버전을
발행해서 반영한다. staging smoke는 판단 수를
