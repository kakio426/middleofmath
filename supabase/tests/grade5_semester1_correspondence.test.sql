...
select is(
  (
    ...
    where anchor_key = '[6수02-01]'
  ),
  0::bigint,
  'the A4-3 review loop does not publish the correspondence anchor'
);
...
select is(
  ...
  0::bigint,
  'the three-unit grade 5 review set still receives no publication allow-list rows'
);
...
select is(
  ...
  0::bigint,
  'the three-unit grade 5 semester 1 review content is not published'
);
...
select is(
  ...
  0::bigint,
  'teacher-only correspondence rationales remain outside the database'
);
...
select throws_ok(
  $$insert into public.diagnosis_sets (
      ...
      'grade5-semester1-correspondence-review-negative',
      '1.0.0',
      'correspondence-review-negative',
      'draft',
      '{}'::jsonb,
      '{
        "manifest": {"grade": 5, "semester": 1},
        "curriculumAnchors": [{"id": "[6수02-01]"}]
      }'::jsonb
    )$$,
  'P0001',
  'curriculum anchor grade or semester mismatch',
  'the unpublished correspondence anchor cannot enter a database diagnosis set'
);
...
select lives_ok(
  ...
  'the shared SQL runtime contract accepts grade 5 correspondence review-shaped content'
);
