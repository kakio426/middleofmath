...
/**
 * Browser와 Node에서 같은 결과를 내는 동기 SHA-256.
 * 발행 게이트는 비동기 Web Crypto를 기다릴 수 없으므로 작은 순수 구현을 쓴다.
 */
...
export function jsonSha256(value: unknown): string {
  return `sha256:${sha256Utf8(JSON.stringify(value))}`;
}
...
export function canonicalJson(value: unknown): string {
  ...
  if (serialized === undefined) {
    throw new Error("checksum에 직렬화할 수 없는 값입니다.");
  }
  return serialized;
}

export function diagnosisContentChecksum(value: {
  manifest: { checksum: string };
}): string {
  const input = {
    ...value,
    manifest: { ...value.manifest, checksum: "" }
  };
  return sha256Utf8(canonicalJson(input));
}
