...
function makeSignal(
  ...
  return {
    ...
    teachingMove,
    parentSummary: `‘${title}’ 과정을 작은 단계로 나누어 연습하고 있습니다.`,
    homePrompt
  };
}
...
const addedAnchors: DiagnosisSet["curriculumAnchors"] = [
  {
    id: "[4수01-08]",
    label: "자연수의 사칙계산이 필요한 상황에서 어림셈하기",
    source
  },
  {
    id: "[4수03-18]",
    label: "1L와 1mL의 관계를 알고 들이를 두 가지 방식으로 나타내기",
    source
  },
  {
    id: "[4수03-21]",
    label: "1kg과 1g의 관계를 알고 무게를 두 가지 방식으로 나타내기",
    source
  },
  {
    id: "[4수03-22]",
    label: "1t을 알고 1t과 1kg의 관계 이해하기",
    source
  },
  {
    id: "[4수03-23]",
    label: "실생활과 연결하여 무게의 덧셈과 뺄셈하기",
    source
  }
];
...
const addedSignals: SignalDefinition[] = [
  makeSignal("multiplication.two-digit-factor", "두 자리 수를 곱하는 부분곱", "십의 자리와 일의 자리에서 생기는 두 부분곱 중 하나를 빠뜨리는 선택이 관찰되었습니다.", "곱하는 수를 십과 일로 나누고 두 곱을 각각 적게 하세요.", "23×12에서 12를 10과 2로 나누어 말해 보게 해주세요."),
  makeSignal("multiplication.estimate", "곱셈 결과 어림", "곱셈의 크기를 자릿수와 연결하는 근거가 더 필요합니다.", "곱하는 수를 가까운 십의 자리 수로 바꾸어 먼저 계산하세요.", "48을 50으로 보았을 때 50×6이 얼마쯤인지 물어봐 주세요."),
  makeSignal("division.meaning", "나눗셈의 뜻과 곱셈 관계", "전체, 묶음 수, 한 묶음의 수를 서로 연결하는 과정이 흔들립니다.", "같은 수만큼 묶어 보고 곱셈식과 나눗셈식을 함께 쓰게 하세요.", "24개를 6개씩 묶으면 몇 묶음인지 함께 놓아보세요."),
  makeSignal("division.remainder-check", "몫과 나머지 확인", "나머지를 나누는 수보다 작게 남기고 원래 수를 확인하는 과정이 안정적이지 않습니다.", "나누는 수×몫+나머지로 원래 수가 되는지 확인하세요.", "29=4×7+1에서 각 수가 무엇을 뜻하는지 물어봐 주세요."),
  makeSignal("division.estimate", "나눗셈의 몫 어림", "나누어지는 수와 나누는 수의 크기로 몫을 가늠하는 근거가 더 필요합니다.", "나누어떨어지는 가까운 수를 먼저 찾아보게 하세요.", "83을 80으로 보아 80÷4를 먼저 구해 보게 해주세요."),
  makeSignal("circle.equal-radii", "한 원의 같은 반지름", "중심에서 원 위까지의 길이가 방향과 관계없이 같다는 성질을 적용하는 과정이 흔들립니다.", "중심에서 원 위의 여러 점까지 직접 재어 비교하세요.", "동전 가운데에서 가장자리까지 여러 방향으로 길이를 비교해 보세요."),
  makeSignal("circle.compass", "컴퍼스로 원 그리기", "컴퍼스의 침, 연필, 벌어진 길이의 역할을 연결하는 근거가 더 필요합니다.", "침은 움직이지 않고 연필 쪽만 한 바퀴 돌리는 순서를 확인하세요.", "컴퍼스를 3cm 벌리면 어떤 길이가 3cm인지 물어봐 주세요."),
  makeSignal("fraction.discrete", "여러 개의 분수만큼", "전체 개수를 같은 묶음으로 나눈 뒤 필요한 묶음을 고르는 과정이 흔들립니다.", "전체를 분모만큼 같은 묶음으로 나누고 분자만큼 고르게 하세요.", "12개를 3묶음으로 나눈 뒤 그중 1묶음의 개수를 물어봐 주세요."),
  makeSignal("fraction.unit", "단위분수", "전체를 똑같이 나눈 한 조각과 단위분수를 연결하는 근거가 더 필요합니다.", "분자는 1로 두고 전체 조각 수를 분모에 놓게 하세요.", "전체를 7조각으로 나눈 한 조각을 분수로 말해 보게 해주세요."),
  makeSignal("fraction.types", "분수의 종류", "분자와 분모의 크기를 보고 분수의 종류를 구분하는 과정이 안정적이지 않습니다.", "분자가 분모보다 작은지, 같거나 큰지를 먼저 표시하세요.", "7/5에서 위 수와 아래 수 중 어느 수가 큰지 물어봐 주세요."),
  makeSignal("fraction.convert", "가분수와 대분수 바꾸기", "분모만큼 묶어 자연수 부분을 만드는 과정이 흔들립니다.", "분자를 분모만큼씩 묶고 남은 수를 따로 표시하세요.", "7개를 3개씩 묶으면 몇 묶음과 몇 개가 남는지 물어봐 주세요."),
  makeSignal("fraction.unit-compare", "단위분수 크기 비교", "같은 전체를 더 많이 나눌수록 한 조각이 작아진다는 관계가 흔들립니다.", "같은 길이의 막대를 서로 다른 수로 나누어 한 조각씩 겹쳐 보세요.", "같은 피자를 3조각과 5조각으로 나눌 때 어느 한 조각이 큰지 물어봐 주세요."),
  makeSignal("measurement.capacity-measure", "들이 측정과 어림", "물건의 들이에 맞는 단위와 어림값을 고르는 근거가 더 필요합니다.", "작은 용기는 mL, 큰 용기는 L로 먼저 나누어 보세요.", "컵과 물통 중 어느 것의 들이를 L로 재기 좋은지 물어봐 주세요."),
  makeSignal("measurement.capacity-arithmetic", "들이의 합과 차", "L끼리, mL끼리 계산하고 단위를 정리하는 과정이 흔들립니다.", "L와 mL를 세로로 나란히 놓고 같은 단위끼리 계산하세요.", "2L 300mL와 1L 500mL를 같은 단위끼리 더해 보게 해주세요."),
  makeSignal("measurement.weight-measure", "무게 측정과 어림", "물건의 무게에 맞는 단위와 어림값을 고르는 근거가 더 필요합니다.", "가벼운 물건은 g, 무거운 생활 물건은 kg으로 먼저 나누어 보세요.", "클립과 가방 중 어느 것을 kg으로 재기 좋은지 물어봐 주세요."),
  makeSignal("measurement.ton", "t과 kg의 관계", "t을 kg으로 바꿀 때 1t=1000kg을 적용하는 과정이 안정적이지 않습니다.", "1t=1000kg을 수직선이나 묶음으로 확인하세요.", "2t은 1000kg이 몇 번인지 물어봐 주세요."),
  makeSignal("measurement.weight-arithmetic", "무게의 합과 차", "kg끼리, g끼리 계산하고 단위를 정리하는 과정이 흔들립니다.", "kg과 g를 세로로 나란히 놓고 같은 단위끼리 계산하세요.", "2kg 400g과 1kg 300g을 같은 단위끼리 더해 보게 해주세요."),
  makeSignal("pictograph.classify-table", "자료 분류와 표", "자료를 종류별로 빠짐없이 세어 표에 옮기는 과정이 흔들립니다.", "자료를 하나씩 표시하며 종류별 칸에 바를 정자로 세게 하세요.", "빨강과 파랑 물건을 나누어 각각 몇 개인지 세어 보세요."),
  makeSignal("pictograph.convert", "그림 수와 실제 수량", "그림 수에 범례의 수를 곱해 실제 수량으로 바꾸는 과정이 안정적이지 않습니다.", "그림을 하나 짚을 때마다 범례만큼 뛰어 세게 하세요.", "그림 한 개가 5일 때 그림 네 개를 5씩 뛰어 세어 보세요."),
  makeSignal("pictograph.complete", "그림그래프 완성", "실제 수량을 범례로 나누어 필요한 그림 수를 정하는 과정이 흔들립니다.", "실제 수량에 범례가 몇 번 들어가는지 묶어 보세요.", "20을 5씩 묶으면 그림이 몇 개 필요한지 물어봐 주세요.")
];
...
const addedJudgments: Judgment[] = [
  ...
  makeJudgment({
    ...
    answers: [{ id: "6", label: "6개" }, { id: "15", label: "15개" }, { id: "21", label: "21개" }]
  }),
  makeJudgment({
...
const fractionMeasurementGraphJudgments: Judgment[] = [
  ...
  makeJudgment({
    ...
    visual: { kind: "none" }, interactionType: "measurement",
  ...
  makeJudgment({
    ...
    prompt: "물통의 들이로 알맞은 것은 무엇일까요?", visual: { kind: "none" }, interactionType: "measurement",
    signalId: "measurement.capacity-measure",
  ...
  makeJudgment({
    ...
    answers: [{ id: "3l800ml", label: "3L 800mL" }, { id: "3l700ml", label: "3L 700mL" }, { id: "1l200ml", label: "1L 200mL" }]
  }),
  makeJudgment({
    ...
    prompt: "남은 물은 얼마일까요?", visual: { kind: "none" }, interactionType: "measurement",
    signalId: "measurement.capacity-arithmetic",
  ...
  makeJudgment({
    ...
    prompt: "알맞은 무게는 무엇일까요?", visual: { kind: "none" }, interactionType: "measurement",
    signalId: "measurement.weight-measure",
  ...
  makeJudgment({
    ...
    prompt: "kg으로만 나타내면 얼마일까요?", visual: { kind: "none" }, interactionType: "measurement",
    signalId: "measurement.ton",
  ...
  makeJudgment({
    ...
    prompt: "남은 쌀의 무게는 얼마일까요?", visual: { kind: "none" }, interactionType: "measurement",
    signalId: "measurement.weight-arithmetic",
  ...
  makeJudgment({
    ...
    prompt: "축구는 ● 5개, 야구는 ● 3개, 농구는 ● 2개예요. 야구와 농구를 합한 수는 축구와 몇 명 차이일까요?",
    visual: { kind: "pictograph", symbol: "●", value: 3, rows: [{ label: "축구", count: 5 }, { label: "야구", count: 3 }, { label: "농구", count: 2 }] }, interactionType: "pictograph",
    signalId: "pictograph.difference",
    answers: [{ id: "0-students", label: "0명" }, { id: "2-students", label: "2명" }, { id: "6-students", label: "6명" }]
  }),
  makeJudgment({
...
const curriculumAnchors = grade3Semester2Diagnosis.curriculumAnchors
  .map((anchor) => {
    if (anchor.id === "[4수03-17]") {
      return { ...anchor, label: "들이의 단위를 알고 들이를 어림하고 재기" };
    }
    if (anchor.id === "[4수03-20]") {
      return { ...anchor, label: "무게의 단위를 알고 무게를 어림하고 재기" };
    }
    return anchor;
  })
  .concat(addedAnchors);
