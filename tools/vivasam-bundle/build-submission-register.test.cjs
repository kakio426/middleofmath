...
test("submission register contains only the received and publicly verified lessons", () => {
  ...
  assert.equal(register.completedRecordCount, 3);
  assert.equal(new Set(register.records.map((record) => record.publicUrl)).size, 3);
  ...
  assert.equal((markdown.match(/^## \d{2}\./gm) || []).length, 3);
  assert.match(markdown, /현재 작성 완료: 3\/30/);
