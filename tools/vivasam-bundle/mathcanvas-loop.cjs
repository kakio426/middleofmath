...
function validateMathCanvasPackageLink(prepared, editorUrl) {
  const lessonId = prepared.bundle.lessonId;
  const expectedSections = editorUrl ? 4 : 3;
  let digest = "";
  for (const packageRoot of lessonPackageRoots(prepared)) {
    ...
    ensure((html.match(/data-section=/g) || []).length === expectedSections, `${lessonId} 공개 페이지 섹션 수가 잘못되었습니다.`);
    if (editorUrl) {
      ensure((html.match(/data-section="mathcanvas"/g) || []).length === 1, `${lessonId} MathCanvas 섹션이 정확히 하나가 아닙니다.`);
      ensure(
      ...
      ensure(!html.includes('data-section="mathcanvas"'), `${lessonId} 준비되지 않은 MathCanvas 링크가 노출되었습니다.`);
    }
  }
  return {
    status: "synced",
