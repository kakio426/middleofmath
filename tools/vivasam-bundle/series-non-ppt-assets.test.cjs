...
test("PPT가 도착한 차시만 활동지 이미지와 공개 패키지를 만든다", async () => {
  ...
  assert.equal(report.lessonCount, 2);
  assert.equal(report.worksheetCount, 2);
  assert.equal(report.packageCount, 2);
  assert.equal(report.supportCount, 2);
  assert.equal(report.representativeImageCount, 2);
  assert.equal(new Set(report.worksheetHashes).size, 2);
  ...
  for (const item of report.items) {
    ...
    const pptAvailable = [1, 2].includes(item.lesson.sequence);
  ...
  for (const lesson of loadSeriesLessons().filter((lesson) => ![1, 2].includes(lesson.sequence))) {
...
test("PPT가 도착한 원본과 Eduitit 동기화 폴더에는 공개 자료만 있다", () => {
  const lessons = loadSeriesLessons().filter((lesson) => [1, 2].includes(lesson.sequence));
  ...
  assert.equal(syncedLessonDirectories.length, 2);
